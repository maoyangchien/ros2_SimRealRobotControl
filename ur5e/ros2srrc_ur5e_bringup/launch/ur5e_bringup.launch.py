...
def generate_launch_description():
    ...
    while (error == True):
        print("     + Option N1: UR5e alone.")
        print("     + Option N2: UR5e in Cylindric Stand.")
        print("     + Option N3: UR5e in Cranfield University IA Lab: Collaborative Table.")
        print("     + Option N4: UR5e in Cranfield University IA Lab: Table.")
        print("     + Option N5: UR5e in Cranfield University IA Lab: Table + Safety Wall.")
    ...
    # ***** ROBOT DESCRIPTION ***** #
    # UR5e Description file package:
    ur5e_description_path = os.path.join(
        get_package_share_directory('ros2srrc_ur5e_gazebo'))
    # UR10e ROBOT urdf file path:
    xacro_file = os.path.join(ur5e_description_path,
                              'urdf',
                              'ur5e.urdf.xacro')
    ...
    ros2_controllers_path = os.path.join(
        get_package_share_directory("ros2srrc_ur5e_bringup"),
        "config",
        "ur_controllers.yaml",
    )
    ...
    if (EE_no == "true"):
        robot_description_semantic_config = load_file(
            "ros2srrc_ur5e_moveit2", "config/ur5.srdf"
        )
    ...
    kinematics_yaml = load_yaml(
        "ros2srrc_ur5e_moveit2", "config/kinematics.yaml"
    )
    robot_description_kinematics = {"robot_description_kinematics": kinematics_yaml}

    # joint_limits.yaml file:
    joint_limits_yaml = load_yaml(
        "ros2srrc_ur5e_moveit2", "config/joint_limits.yaml"
    )
    ...
    pilz_cartesian_limits_yaml = load_yaml(
        "ros2srrc_ur5e_moveit2", "config/pilz_cartesian_limits.yaml"
    )
    ...
    ompl_planning_yaml = load_yaml(
        "ros2srrc_ur5e_moveit2", "config/ompl_planning.yaml"
    )
    ompl_planning_pipeline_config["move_group"].update(ompl_planning_yaml)

    # MoveIt!2 Controllers:
    moveit_simple_controllers_yaml = load_yaml(
        "ros2srrc_ur5e_moveit2", "config/ur_controllers.yaml"  
    )
